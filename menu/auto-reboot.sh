...
CEKEXPIRED () {
    ...
    Exp1=$(curl -sS https://raw.githubusercontent.com/riskanuris2/VPN-Master/main/permission/ip| grep $MYIP | awk '{print $3}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl -sS https://raw.githubusercontent.com/riskanuris2/VPN-Master/main/permission/ip| awk '{print $4}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
